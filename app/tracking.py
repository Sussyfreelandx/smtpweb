    ...
    try:
        ...
        if recipient and recipient.campaign_id == campaign_id:
            if not recipient.opened_at: # Record only the first open
                ...
                # Don't overwrite status if already clicked or unsubscribed
...
def unsubscribe(campaign_id, recipient_id):
    ...
    try:
        ...
        if recipient and recipient.campaign_id == campaign_id:
            ...
            # We don't have an unsubscribed_at field in the model I gave you earlier, 
            # so we just update status. If you want a timestamp, we can add it later.
            db.session.commit()
