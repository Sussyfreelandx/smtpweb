...
# NOTE: The global proxy patching code has been removed from here.
# It is now handled centrally in wsgi.py / run.py to prevent RecursionError.
# This file now assumes the socket/smtp environment is already patched.
            ...
            while True:
                ...
                for res in results:
                    ...
                    if success:
                        ...
                        # Webhook
                        try:
                            # Safely import to avoid circular dependency issues
                ...
                # Broadcast progress
                try:
                    # Adjusted import to avoid potential import errors
                    ...
                    broadcast_campaign_progress(campaign_id, total_sent, total_failed, total, f"Batch finished via {current_handler.smtp_server}")
                     ...
                     # Fallback if app/events.py or app/main/events.py structure varies
                     pass
                except Exception as e:
                ...
                if campaign and campaign.status == 'Sending':
                    ...
                    if remaining > 0 and delay_seconds > 0:
                        # Eventlet-friendly sleep is automatically handled if monkey_patch is called in wsgi.py
